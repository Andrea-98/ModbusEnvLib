...
public class ModbusServer extends Thread {
    ...
    private void ReadCoils(java.net.Socket socket) {
        ...
        if ((receiveData.startingAdress + 1 + receiveData.quantity) > 65535) // Invalid Starting adress or Starting
                                                                             // address + quantity
        {
            sendData.errorCode = (byte) (receiveData.functionCode + 0x80);
        ...
        System.arraycopy(coils, receiveData.startingAdress + 1, sendData.sendCoilValues, 0,
                sendData.sendCoilValues.length);
    ...
    private void ReadDiscreteInputs(java.net.Socket socket) {
        ...
        sendData.sendCoilValues = new boolean[receiveData.quantity];
        System.arraycopy(discreteInputs, receiveData.startingAdress + 1, sendData.sendCoilValues, 0,
    ...
    private void ReadHoldingRegisters(java.net.Socket socket) {
        ...
        System.arraycopy(holdingRegisters, receiveData.startingAdress + 1, sendData.sendRegisterValues, 0,
                receiveData.quantity);
    ...
    private void ReadInputRegisters(java.net.Socket socket) {
        ...
        sendData.byteCount = (short) (2 * receiveData.quantity);
        sendData.sendRegisterValues = new int[receiveData.quantity];
        System.arraycopy(inputRegisters, receiveData.startingAdress + 1, sendData.sendRegisterValues, 0,
                receiveData.quantity);
    ...
    private void WriteSingleRegister(java.net.Socket socket) {
        ...
        if ((receiveData.receiveRegisterValues[0] < 0x0000) | (receiveData.receiveRegisterValues[0] > 0xFFFF)) // Invalid
                                                                                                               // Value
        {
            sendData.errorCode = (byte) (receiveData.functionCode + 0x80);
            sendData.exceptionCode = 3;
    ...
    private void WriteMultipleCoils(java.net.Socket socket) {
        ...
        if (((int) receiveData.startingAdress + 1 + (int) receiveData.quantity) > 65535) // Invalid Starting adress or
     ...
     * 
     * @param value Implementation of Interface
     *              INumberOfConnectedClientsChangedDelegator
     */
    ...
    /**
     * Gets the Client connection timeout, which disconnects a connection to a
     * client
     * 
    ...
    /*
     * Sets the Client connection timeout, which disconnects a connection to a
     * client
     * 
